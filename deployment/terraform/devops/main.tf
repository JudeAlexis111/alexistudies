...
module "state_bucket" {
  ...
  name       = "alexistudies-dev-terraform-state"
...
resource "google_project_iam_binding" "devops_owners" {
  ...
  role    = "roles/owner"
  members = ["group:alexistudies-dev-devops-owners@clinicalstudies.one"]
}

# Org level IAM permissions for org admins.
resource "google_folder_iam_member" "admin" {
  folder = "folders/613190869381"
  role   = "roles/resourcemanager.folderAdmin"
  member = "group:alexistudies-dev-folder-admins@clinicalstudies.one"
}
